...
- Google Workspaceは利用者が承認した `totono-niwa.com` の組織で管理する。過去メールの移行は不要で、新規運用。旧候補のドメイン契約やMicrosoft 365には手を加えない。
- `himawari-fc.jp` をセカンダリドメインとして追加し、ムームーDNSにGoogle確認用TXTを追加。Googleの「ドメイン所有権の確認が完了しました」を確認済み。
- 既存のWeb用ALIAS `ct8m36st.up.railway.app` と `_railway-verify` TXTは保持。追加後もサイトHTTP 200と確認用TXTの公開DNS反映を確認。新規メール用のMX・SPF・DKIMはまだ追加していない。
- 独立した受信箱を用意するためのユーザー追加は、現在の契約料金を管理画面で確認済み。追加課金の承認待ちで、ユーザーは未作成。Google認証情報も未作成。RailwayのCLIと接続アプリは再認証待ち。本番の送信機能はまだ有効化していない。
